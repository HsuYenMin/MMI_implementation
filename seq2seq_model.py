...
class Seq2SeqModel(object):
  ...
  def __init__(self, source_vocab_size, target_vocab_size, buckets, size,
    ...
    def seq2seq_TS(encoder_inputs,decoder_inputs,do_decode):
        if attention:
            print "se2seq_TS Attention Model"
            return embedding_attention_seq2seq(
    ...
    if forward_only:
        if beam_search:
              ...
              self.outputs, self.losses = model_with_buckets(
                  ...
                  lambda w, z: seq2seq_TS(w, z,True),softmax_loss_function=softmax_loss_function,MMIparam=MMIweight)
    ...
    if not forward_only:
      ...
      for b in xrange(len(buckets)):
        gradients_ST = tf.gradients(self.losses[0][b], params_ST)
    ...
    if not forward_only:
      ...
      opt = tf.train.GradientDescentOptimizer(self.learning_rate)
      for b in xrange(len(buckets)):
        gradients_TS = tf.gradients(self.losses[1][b], params_TS)
  ...
  def step_ST(self, session, encoder_inputs_ST, decoder_inputs_ST, target_weights_ST,
    ...
    if not forward_only:
      output_feed_ST = [self.updates_ST[bucket_id],  # Update Op that does SGD.
                     ...
                     self.losses[0][bucket_id]]  # Loss for this batch.
    else:
        if beam_search:
              output_feed_ST = [self.beam_path[bucket_id]]  # Loss for this batch.
              output_feed_ST.append(self.beam_symbol[bucket_id])
        else:
            output_feed_ST = [self.losses[0][bucket_id]]

        for l in xrange(decoder_size):  # Output logits.
            output_feed_ST.append(self.outputs[0][bucket_id][l])
    # print bucket_id
  ...
  def step_TS(self, session, encoder_inputs_TS, decoder_inputs_TS, target_weights_TS,
    ...
    if not forward_only:
      output_feed_TS = [self.updates_TS[bucket_id],  # Update Op that does SGD.
                     ...
                     self.losses[1][bucket_id]]  # Loss for this batch.
    else:
        if beam_search:
              output_feed_TS = [self.beam_path[bucket_id]]  # Loss for this batch.
              output_feed_TS.append(self.beam_symbol[bucket_id])
        else:
            output_feed_TS = [self.losses[1][bucket_id]]

        for l in xrange(decoder_size):  # Output logits.
            output_feed_TS.append(self.outputs[1][bucket_id][l])
    # print bucket_id
  ...
  # Check if the sizes match.
    ...
    if not forward_only:
      output_feed_MMI = [self.updates_MMI[bucket_id],  # Update Op that does SGD.
                     ...
                     self.losses[2][bucket_id]]  # Loss for this batch.
    else:
        if beam_search:
              output_feed_MMI = [self.beam_path[bucket_id]]  # Loss for this batch.
              output_feed_MMI.append(self.beam_symbol[bucket_id])
        else:
            output_feed_MMI = [self.losses[2][bucket_id]]

        for l in xrange(decoder_size):  # Output logits.
            output_feed_MMI.append(self.outputs[0][bucket_id][l])
    # print bucket_id
